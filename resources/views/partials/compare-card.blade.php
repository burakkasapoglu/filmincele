...
<div class="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
    <div class="h-44 relative">
        @if($m['backdrop_path'])
            <img src="https://image.tmdb.org/t/p/w780{{ $m['backdrop_path'] }}" class="w-full h-full object-cover">
    ...
    <div class="p-5 -mt-14 relative">
        <div class="flex gap-4 items-end mb-4">
            @if($m['poster_path'])<img src="https://image.tmdb.org/t/p/w185{{ $m['poster_path'] }}" class="w-24 rounded-xl shadow-xl flex-shrink-0">@endif
            <div>
            ...
            <div class="mb-3"><p class="text-gray-500 text-[10px] uppercase mb-1">Yönetmen</p>
                @foreach($directors as $d)
                    <a href="{{ kisi_url($d['id'], $d['name']) }}" class="inline-flex items-center gap-1.5 bg-gray-800/50 rounded-lg px-2.5 py-1 hover:bg-gray-800 transition group mr-1.5 mb-1">
                        <div class="w-5 h-5 rounded-full overflow-hidden bg-gray-700 flex-shrink-0">@if($d['profile_path'])<img src="https://image.tmdb.org/t/p/w92{{ $d['profile_path'] }}" class="w-full h-full object-cover">@else<span class="text-[10px]">🎬</span>@endif</div>
