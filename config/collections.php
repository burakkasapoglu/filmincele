<?php

return [
    'marvel' => [
        'name' => 'Marvel Sinematik Evreni',
        'description' => "Iron Man'den Avengers: Endgame'e tüm Marvel filmleri",
        'emoji' => '🦸',
        'movies' => [
            1726, 1724, 10138, 10195, 1771, 24428, 68721, 76338, 100402,
            118340, 99861, 102899, 271110, 284052, 283995, 315635, 284053,
            284054, 299536, 363088, 299534, 429617, 566525, 497698, 634649,
        ],
    ],
    'lotr' => [
        'name' => 'Yüzüklerin Efendisi & Hobbit',
        'description' => 'Orta Dünya destanı',
        'emoji' => '🧙',
        'movies' => [120, 121, 122, 49051, 57158, 122917],
    ],
    'star-wars' => [
        'name' => 'Star Wars Evreni',
        'description' => 'Uzak bir galakside tamamlanan destan',
        'emoji' => '🌟',
        'movies' => [11, 1891, 1892, 1893, 1894, 1895, 181808, 330459, 348350],
    ],
    'harry-potter' => [
        'name' => 'Harry Potter Serisi',
        'description' => 'Büyücülük dünyasının 8 filmi',
        'emoji' => '⚡',
        'movies' => [671, 672, 673, 674, 675, 767, 12444, 12445],
    ],
    'dark-knight' => [
        'name' => 'Kara Şövalye Üçlemesi',
        'description' => "Christopher Nolan'ın Batman üçlemesi",
        'emoji' => '🦇',
        'movies' => [272, 155, 49026],
    ],
    'jurassic' => [
        'name' => 'Jurassic Park & World',
        'description' => 'Dinozorlar geri döndü',
        'emoji' => '🦖',
        'movies' => [329, 330, 331, 135397, 351286, 507086],
    ],
    'fast-furious' => [
        'name' => 'Hızlı ve Öfkeli Serisi',
        'description' => 'Aile ve hız tutkusu',
        'emoji' => '🏎️',
        'movies' => [9799, 584, 9615, 13804, 51497, 82992, 168259, 337339, 385128, 385687],
    ],
    'mission-impossible' => [
        'name' => 'Mission: Impossible',
        'description' => 'Tom Cruise ile imkansız görevler',
        'emoji' => '💣',
        'movies' => [954, 955, 956, 56292, 177677, 353081, 575264, 575265],
    ],
    'pixar' => [
        'name' => 'Pixar Klasikleri',
        'description' => 'En sevilen Pixar animasyonları',
        'emoji' => '✨',
        'movies' => [862, 863, 12, 585, 10193, 14160, 2062, 10681, 9487],
    ],
    'must-watch' => [
        'name' => 'Mutlaka İzlenmesi Gerekenler',
        'description' => 'Sinema tarihinin en iyi filmleri',
        'emoji' => '👑',
        'movies' => [
            278, 238, 240, 424, 389, 129, 155, 680, 550, 769,
            510, 13, 497, 807, 274, 1422, 157336, 1124, 603, 27205,
            1402, 406, 629, 330, 329, 78, 98, 122, 694, 857,
        ],
    ],
];
